...
func handleRequests(addChan chan com.TimeRequest, delChan chan com.TimeReply, done chan bool,numIt int,requestTmp int) {
	...
	for {
		...
		case reply := <-delChan:
			fmt.Println(reply.Id, " ", reply.T.Sub(requests[reply.Id]).Seconds())
			delete(requests, reply.Id)
			cont = cont + 1
			if (cont==numIt*requestTmp){
				done <- true
			}
		}
	}
...
func sendFinal(endpoint string, interval com.TPInterval) {
	tcpAddr, err := net.ResolveTCPAddr("tcp", endpoint)
	checkError(err)
